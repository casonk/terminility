...
3. **Session Management Layer** (`sessions.sh`)
   ...
   - Creates sessions idempotently: skips any session that already exists (`tmux has-session`).
   - Ensures the tmux server is running before session creation (`tmux start-server`).
   - Writes `~/.terminility_aliases` (fully regenerated each run) with one alias per session.
...
/mnt/4tb-m2/git/**/.git    ← scanned by sessions.sh
         │
         ▼
tmux sessions (one per repo, idempotent)
         │
         ▼
~/.terminility_aliases      ← fully regenerated each run
         │
         ▼
...
| `bash sessions.sh` | Rescan repos, create new sessions, regenerate aliases (safe to re-run anytime) |
...
- **Environment overrides**: `TERMINILITY_GIT_BASE` and `TERMINILITY_ALIAS_FILE` let users redirect scanning and output without modifying scripts.
...
- **Alias file atomicity**: `~/.terminility_aliases` is written in a single redirect (`>`), not appended, so repeated runs never accumulate duplicates.
